...
bash /home/muzasyed/projects/project2/undo.sh
...
for filepath in $pattern; do
  if [ -f "$filepath" ]; then
    ...
    thirdDim=("${args[1]}")
    echo "Compiling ${ITER}"
    iree-compile $filepath -o $SRC_DIR/$file.${ITER}.vmfb --iree-hal-target-device=hip --iree-hip-target=gfx942 --mlir-print-ir-after-all --debug-only=iree-llvmgpu-kernel-config &> $SRC_DIR/$file.${ITER}.mlir
    echo "Running ${ITER}"
...
ITER="mine"
echo "Setting up ${ITER}"
bash /home/muzasyed/projects/project2/setup.sh
...
for filepath in $SRC_DIR/*_f16*_tB.mlir; do
  if [ -f "$filepath" ]; then
    file=${filepath##*/}
    [[ $file =~ ([0-9]*)_([0-9]*)_([0-9]*)_.*$ ]]
    args=("${BASH_REMATCH[@]}")
...
# echo "$shape"
# iree-compile ${ITER} -o $SRC_DIR --iree-hal-target-device=hip --iree-hip-target=gfx942 --mlir-print-ir-after-all
